...
SimpleForm.setup do |config|
  ...
  # horizontal input for inline radio buttons and check boxes
  config.wrappers :horizontal_radios, item_wrapper_class: 'form-check form-check-inline horizontal_radios', tag: 'div', class: '', error_class: 'form-group-invalid', valid_class: 'form-group-valid' do |b|
    b.use :html5
    b.optional :readonly
  ...
  config.wrappers :inline_form, tag: 'span', class: 'col-12' do |b|
    b.use :html5
    b.use :placeholder
    b.optional :maxlength
    b.optional :minlength
  ...
  config.wrappers :inline_boolean, tag: 'span', class: 'form-check flex-wrap justify-content-start me-sm-2', error_class: 'form-group-invalid', valid_class: 'form-group-valid' do |b|
    b.use :html5
    b.optional :readonly
    b.use :input, class: 'form-check-input', error_class: 'is-invalid'
...
class SwitchInput < SimpleForm::Inputs::BooleanInput; end
class DateTimeInput < SimpleForm::Inputs::DateTimeInput

  def input(wrapper_options = nil)
    merged_input_options = merge_wrapper_options(input_html_options, wrapper_options).merge(autocomplete: 'off')
...
class CurrencyInput < SimpleForm::Inputs::Base
  def input(wrapper_options)
    ...
    opc_prepend = options.dig(:input_html, :remove_prepend)
    size = options.dig(:size)
    size_class = size.present? ? "input-group-#{size}" : ""

    currency = options.delete(:currency) || "R$"
    merged_input_options = merge_wrapper_options(input_html_options, wrapper_options)

    tag.div input_group(currency, merged_input_options, opc_prepend), class: "input-group #{size_class}"
  end

  private

  def input_group(currency, merged_input_options, opc_prepend = false)
    addon = opc_prepend ? '' : currency_addon(currency)
    "#{addon} #{@builder.text_field(attribute_name, merged_input_options)}".html_safe
  end

  def currency_addon(currency)
    tag.span(currency, class: 'input-group-text')
  end
...
class PercentageInput < SimpleForm::Inputs::Base
  ...
  def symbol_addon(symbol)
    tag.div tag.div(symbol, class: 'input-group-text')
  end
...
class Select2AjaxInput < SimpleForm::Inputs::CollectionSelectInput
  def input(wrapper_options)
    ...
    initial_option = options.delete(:initial_option)
    ...
    super(merge_wrapper_options(opts, wrapper_options))
  end
end
